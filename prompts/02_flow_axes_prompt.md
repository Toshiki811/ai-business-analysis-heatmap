...
- `category_flow_columns[].steps[]` は、個別タスク名に寄せすぎず、前回成果物と同程度の抽象工程粒度にする。例: `受付・起案`、`内容確認`、`情報収集・照合`、`計算・作成`、`システム入力`、`承認・決裁`、`実行・支払`、`記録・保管`、`報告・提出`。
- `マトリクス横軸` は業務種別ごとの実作業順を表すが、`経理担当者任命`、`職務分掌確認`、`帳簿一覧整備` のようなタスク名に近い粒度へ具体化しない。
- 例: `経理体制・帳簿管理` の中に `経理体制整備` と `帳簿・帳票管理` がある場合、`経理体制整備` の `steps[]` は `受付・起案` / `内容確認` / `承認・決裁` / `記録・保管`、`帳簿・帳票管理` の `steps[]` は `記録・保管` / `内容確認` / `システム入力` / `報告・提出` のように業務種別別に分ける。
